...
class BuildingStats {
    ...
    initializeOverlay() {
        ...
        // 'Show hidden buildings' checkbox
    ...
    // ------------------------------------------------------------------
    // NEW: Separate function for normal GbG stats
    // ------------------------------------------------------------------
    getGbGBoostForEntity(entity, era, wantedBoost) {
        let total = 0;
        for (const age of ["AllAge", era]) {
            const boosts = entity?.components?.[age]?.boosts?.boosts || [];
            for (const boost of boosts) {
                // Only count battleground or all
                if (
                    (boost.targetedFeature === "all" ||
                     boost.targetedFeature === "battleground")
                    && boost.type === wantedBoost
                ) {
                    total += boost.value;
                }
            }
            // Also check abilities for the same features
            if (entity.abilities) {
                for (const ability of entity.abilities) {
                    ...
                    for (const hint of hints) {
                        ...
                        if (
                            (hintMap[age].targetedFeature === "all" ||
                             hintMap[age].targetedFeature === "battleground")
                            && hintMap[age].type === wantedBoost
                        ) {
    ...
    // ------------------------------------------------------------------
    // NEW: Separate function for QI stats (guild_raids)
    // ------------------------------------------------------------------
    getQIBoostForEntity(entity, era, wantedBoost) {
        ...
        for (const age of ["AllAge", era]) {
            const boosts = entity?.components?.[age]?.boosts?.boosts || [];
            for (const boost of boosts) {
                // Only count guild_raids
                if (
                    boost.targetedFeature === "guild_raids" &&
                    boost.type === wantedBoost
                ) {
                    total += boost.value;
                }
            }
            // If you also want "all" to count for QI, uncomment:
            ...
            if (entity.abilities) {
                for (const ability of entity.abilities) {
                    ...
                    for (const hint of hints) {
                        ...
                        if (!hintMap || !hintMap[age]) continue;
                        if (
                            hintMap[age].targetedFeature === "guild_raids" &&
                            hintMap[age].type === wantedBoost
                        ) {
                            total += hintMap[age].value;
                        }
    ...
    calculateTotals(buildingEntries) {
        ...
        buildingEntries.forEach(({ Aoffense, Adefense, Doffense, Ddefense,
                                   QI_Aoffense, QI_Adefense, QI_Doffense, QI_Ddefense,
                                   quantity, cityentity_id }) => {
            if (!GBs && cityentity_id.indexOf("Landmark") >= 0) {
                return;
            }
            // Increment normal GbG stats
            ...
            // Increment QI stats
            totalQI_AOffense += QI_Aoffense * quantity;
            totalQI_ADefense += QI_Adefense * quantity;
            totalQI_DOffense += QI_Doffense * quantity;
            totalQI_DDefense += QI_Ddefense * quantity;
        });
    
        this.overlay.find(".totals").html(
            " Player: <span class='" + (this.stats.isOther ? "player-name other" : "player-name own") + "'>"
            + this.stats.currentPlayer + "</span> | Era: " + this.stats.era
            + "<br/>GbG: A. Offense: " + totalAOffense
            + " | A. Defense: " + totalADefense
            + " | D. Offense: " + totalDOffense
            + " | D. Defense: " + totalDDefense
            + "<br/>QI: A. Offense: " + totalQI_AOffense
            + " | A. Defense: " + totalQI_ADefense
            + " | D. Offense: " + totalQI_DOffense
            + " | D. Defense: " + totalQI_DDefense
            + (GBs ? "" : "<br/><i><small>Not including GBs</small></i>")
        );

        if (typeof window.statTracking != 'undefined') {
            window.statTracking({
                player: this.stats.currentPlayer,
                era: this.stats.era,
                AOffense: this.totalAOffense,
                ADefense: this.totalDDefense,
                DOffense: this.totalDOffense,
                DDefense: this.totalDDefense
            });
        }
    }
    ...
    // ------------------------------------------------------------------
    // getBuildingInfo(...) now uses two separate calls for GbG & QI
    // ------------------------------------------------------------------
    getBuildingInfo(cityentity_id) {
        const entity = MainParser.CityEntities[cityentity_id];
        if (!entity) {
            throw new Error(`Unknown cityentity_id '${cityentity_id}'`);
        }
        ...
        // Retrieve normal GbG attacker stats
        let att_boost_attacker = this.getGbGBoostForEntity(entity, this.stats.era, "att_boost_attacker");
        let att_def_boost_attacker = this.getGbGBoostForEntity(entity, this.stats.era, "att_def_boost_attacker");
        let def_boost_attacker = this.getGbGBoostForEntity(entity, this.stats.era, "def_boost_attacker");
        Aoffense = att_boost_attacker + att_def_boost_attacker;
        Adefense = def_boost_attacker + att_def_boost_attacker;

        // Retrieve normal GbG defender stats
        let att_boost_defender = this.getGbGBoostForEntity(entity, this.stats.era, "att_boost_defender");
        let att_def_boost_defender = this.getGbGBoostForEntity(entity, this.stats.era, "att_def_boost_defender");
        let def_boost_defender = this.getGbGBoostForEntity(entity, this.stats.era, "def_boost_defender");
        Doffense = att_boost_defender + att_def_boost_defender;
        Ddefense = def_boost_defender + att_def_boost_defender;

        // Retrieve QI attacker stats
        ...
        // If the building is a Landmark or X_AllAge_EasterBonus4, your old code adds extra stats
        // for normal GbG only (like advanced_tactics, etc.). We keep that logic as-is.
        ...
        let FP = this.getGbGBoostForEntity(entity, this.stats.era, "fp");
        // If you'd prefer to keep "fp" calculation as before, you could
        // replicate it in a separate function. For now, we reuse the GbG style.
    ...
    displayInventory() {
        ...
        header.html(`
            ...
            <div style='float:left;'>
                ...
                <label for='inv-downgrade' onclick='$(this).prev().click()' title="If selected, temporary buildings will show in their downgraded form so that you're looking at long-term value rather than temporary value. This is highly recommended."> Downgrade temporary buildings</label>
                <input type=checkbox name='inv-neo-max' id='inv-neo-max'>
                <label for='inv-neo-max' onclick='$(this).prev().click()' title="If selected, incomplete Neo buildings (such as neo sentinal outpost l1, neo king l1) will not show, they will only show once you can build them to the max level.  Since this tool is not able to recommend upgrading buildings that are already in your city, this is recommended."> Hide Neo buildings under max level</label>
            ...
            <!--
                <span class='if-other-player'>
                    <button title="Import the other player's inventory" id="imp-inv">Import Inventory</button>
                </span>
                <span class='if-self'>
                    <button title="Exports your inventory to send to another player to receive help with it. This will let them see your inventory if you send it to them." id="exp-inv">Export Inventory</button>
                </span>
            -->
    ...
    addInventoryItem(assetName, quantity=1, name=undefined, relatedUpgradeKit=undefined) {
        ...
        if (MainParser.SelectionKits[assetName]) {
            ...
            for (let x = 0; x < kitItems.length; x++) {
                ...
                if (
                    assetName == "selection_kit_epic_GR24_2" &&
                ...
                if (MainParser.BuildingUpgrades[kitItem.itemAssetName]) {
                    for (let y = 0; y < kitItems.length; y++) {
                        if (
                            MainParser.BuildingUpgrades[kitItem.itemAssetName]?.upgradeSteps[0]?.buildingIds?.[0]
                            == kitItems[y].itemAssetName
                        ) {
                            upgradeKits.push({
                                kit: kitItem.itemAssetName,
                                mainItem: kitItems[y].itemAssetName,
                                upgradeItem: kitItem.itemAssetName,
                                quantity: quantity
                            });
                        }
                    }
    ...
    applyInventoryUpgrades() {
                ...
                if (quantity > 0) do {
                    ...
                    for (let x = 1; x < steps.length; x++) {
                        ...
                        if (this.inventory[current[0]]?.quantity) {
                            if (
                                this.inventory[current[0]].fromKitsQuantity == this.inventory[current[0]].quantity &&
                                this.inventory[current[0]].quantity == 1 &&
                                this.inventory[current[0]].fromKits[0].upgradeItem == kitId
                            ) {
                                continue;
                            }
                            quantity--;
        ...
        for (let assetId in this.inventory) {
            ...
            if (this.settings.neoMaxOnly
                ...
                if (
                    (this.inventory[assetId].name.substr(4, 3) == "Col" && lv < 10)
                    || lv == 1
                ) {
            ...
            if (
                !this.inventory[assetId].quantity
                || !MainParser.CityEntities[assetId]
            ) {
            ...
            if (
                ((sort == "FP" || sort == "FPefficiency") && stats.FPefficiency < 0.25)
