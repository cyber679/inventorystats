...
const icon = (key, title = '') =>
    `<img src='${images(key)}'${title ? ` alt='${title}' title='${title}'` : ''} style='width:20px;height:20px;vertical-align:middle;'>`;
...
class BuildingStats {
    ...
    initializeDefaults() {
		...
		this.settings.splitAllies ??= false;
    }
    ...
    initializeOverlay() {
        ...
        const overlay = this.overlay = $(`<div id='stats-overlay' class='${this.shouldInventoryDisplay()?"inventory":""} ${this.stats.isOther?"other":""}'>
          <div id='building-stats'>
            <div id='stats-header'>
				...
				<input type=checkbox id='split-allies'> <label style='margin-left:5px;' onclick='$(this).prev().click()'>Show Allies</label>
            ...
            <div id='building-body'>
                <table id='stats-table' class='stats-data'>
					<thead><tr>
					    ...
					    <th attr="AO">${icon('attacking_attack_gbg')}</th>
					    <th attr="AD">${icon('attacking_defense_gbg')}</th>
					    <th attr="Aefficiency">${icon('combinedGbGAttacking')}</th>
					    <th attr="DO">${icon('defending_attack_gbg')}</th>
					    <th attr="DD">${icon('defending_defense_gbg')}</th>
					    <th attr="Defficiency">${icon('combinedGbGDefending')}</th>
					    <th attr="totalEfficiency">${icon('combinedGbGAttackingandDefending')}</th>
		...
		overlay.find("#split-allies")
		    .prop("checked", this.settings.splitAllies || false)
		    .change(function() {
		        self.settings.splitAllies = $(this).is(":checked");
		        self.displayBuildings();
		    });
    }
    ...
    calculateTotals(buildingEntries) {
        ...
        // Merge in ally stats
        if (!this.settings.splitAllies) addAllyStats(totals);
